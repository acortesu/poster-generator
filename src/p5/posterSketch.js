...
export default function posterSketch(p) {
  ...
  function getWeightedColor() {
    ...
    if (randomNum < (sum += colorConfig.yellowWeight)) return "#ffb700";
    if (randomNum < (sum += colorConfig.orangeWeight)) return "#e02d25";
    if (randomNum < (sum += colorConfig.blackWeight)) return "#000000";
    if (randomNum < (sum += colorConfig.beigeWeight)) return "#f9dcbc";
    if (randomNum < (sum += colorConfig.blueWeight)) return "#0066be";
    return "#000000";
  }
  ...
  function drawArc(x, y, quadrant, size) {
    ...
    switch (quadrant) {
      ...
      case 2:
        p.arc(x, y, size * 2 * scaleFactor, size * 2 * scaleFactor, 0, p.HALF_PI);
        break;
